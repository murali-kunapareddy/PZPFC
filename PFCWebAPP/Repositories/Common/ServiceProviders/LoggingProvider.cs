...
namespace PFCWebAPP.Repositories.Common.ServiceProviders
{
    public class LoggingProvider : ILoggingProvider
    {
        ...
        public LoggingProvider(IHttpContextAccessor contextAccessor)
        {
        ...
        public string GetUserSESA()
        {
            try
            {
                ...
                {
                    if (httpContextAccessor.HttpContext.Session.GetString("isFromApi") != null || httpContextAccessor.HttpContext.Session.GetString("isFromApi") == "true")
                    {
                        return httpContextAccessor.HttpContext.Session.GetString("ApiSession").ToString() + " (SESSION) ";
                    }
                    if (httpContextAccessor.HttpContext.Session.GetString("userSESA") != null)
                    {
                        UserSESA = httpContextAccessor.HttpContext.Session.GetString("userSESA");
                        return UserSESA + " (SESSION) ";
                    }

                    if (httpContextAccessor.HttpContext.Session.GetString("JWTToken") != null)
                    {
                        JWTToken = httpContextAccessor.HttpContext.Session.GetString("JWTToken").ToString();
                        ...
                        if (JWTToken != null)
                        {
                            ...
                            {
                                var JWTobjUserDetails = new UserInfo().GetUserDetails(tkn);

                                //jsonString = JsonConvert.SerializeObject(JWTobjUserDetails);
                               // logger.Trace("Data From JWTToken and Storing In Session:  " + jsonString);

                                if (httpContextAccessor.HttpContext  != null)
                                {
                                    httpContextAccessor.HttpContext.Session.SetString("userSESA", JWTobjUserDetails.EmployeeSESA);
                              
                                }
                                return JWTobjUserDetails.EmployeeSESA + " (JWT) ";
                            }
                            else
                                return "-";
                        }
                ...
                {
                    if (UserSESA == null || UserSESA == null)
                    {
                        return "-";
                    }
                    else
