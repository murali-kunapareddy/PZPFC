namespace PFCWebAPP.Repositories.PriceList.Models.IntermediateModels
{
    public class DiscountsPH
    {
        public string CustNo { get; set; }
        public string Table { get; set; }
        public string KAPPL { get; set; }
        public string KSCHL { get; set; }
        public string VKORG { get; set; }
        public string VTWEG { get; set; }
        public string SPART { get; set; }
        public string HIENR { get; set; }
        public string KVGR1 { get; set; }
        public string KVGR2 { get; set; }
        public string KVGR3 { get; set; }
        public string PRODH1 { get; set; }
        public string PRODH2 { get; set; }
        public string PRODH3 { get; set; }
        public string PRODH4 { get; set; }
        public string PRODH5 { get; set; }
        public string PRODH6 { get; set; }
        public string KFRST { get; set; }
        public DateTime DATBI { get; set; }
        public DateTime DATAB { get; set; }
        public string KBSTAT { get; set; }
        public string KNUMH { get; set; }
        public int KSTBM { get; set; }
        public float KBETR { get; set; }
    }
}
