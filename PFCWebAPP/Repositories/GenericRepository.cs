...
namespace PFCWebAPP.Repositories
{
    /// <summary>
    /// GenericRepository
    /// </summary>
    ...
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : class
    {
        ...
        public bool InsertCustomerSettingsWithTransaction(List<TEntity> entitiesToAdd, int batchSize)
        {
            if (entitiesToAdd == null || entitiesToAdd.Count == 0) return false;

            using var transaction = _dbContext.Database.BeginTransaction();
            try
            {
                var param = new SqlParameter("@TableName", "MST_CustomerSettings");
                ExecuteQuery("TRUNCATE TABLE MST_CustomerSettings", param);

                _dbContext.BulkInsert(entitiesToAdd, new BulkConfig
                {
                    BatchSize = batchSize,
        ...
        public bool InsertMaterialistWithTransaction(List<TEntity> entitiesToAdd, int batchSize)
        {
            if (entitiesToAdd == null || entitiesToAdd.Count == 0) return false;
            ...
            try
            {
                ...
                ExecuteQuery("TRUNCATE TABLE MST_MaterialMaster", param);
        ...
        //public bool InsertCustomerSettingsWithTransaction(List<TEntity> entitiesToAdd , int BactchSize)
        //{
        //    if (entitiesToAdd == null || entitiesToAdd.Count == 0) return false;
        ...
        //        foreach (var chunk in entitiesToAdd.Chunk(BactchSize))
        //        {
        //            _dbSet.AddRange(chunk);
        //            _dbContext.SaveChanges();
