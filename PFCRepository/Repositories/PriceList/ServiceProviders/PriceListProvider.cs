...
namespace PFCRepository.Repositories.PriceList.ServiceProviders
{
    public class PriceListProvider : IPriceListProvider
    {
        ...
        public PriceListProvider(ILoggingProvider objLoggingProvider, IPriceListRepository objPriceListRepository, IConfigureRepository objConfigureRepository,  ICommonProvider objCommonProvider, INotificationProvider objNotificationProvider)
        {
        ...
        public QueueModel GetQueueModel()
        {
            try
            {
                return _objPriceListRepository.QueueModelRepository.GetManyQueryable(o => o.CreatedBy.StartsWith("API") && o.IsActive == true).OrderBy(o => o.QueueId).FirstOrDefault();
            }
            catch (Exception ex)
            {
                _objLoggingProvider.LogException("Error while GetQueueModel ", ex);
                throw;
            }
        }

        public List<QueueHistory> GetQueueHistory(string CreatedBy, int UserConfigId)
        {
            try
            {
                if (CreatedBy.StartsWith("API"))
                {
                    return _objPriceListRepository.QueueHistoryRepository.GetManyQueryable(o => o.CreatedBy == CreatedBy).ToList();
                }
                return _objPriceListRepository.QueueHistoryRepository.GetManyQueryable(o => o.UserConfigId == UserConfigId).ToList();
            }
            catch (Exception ex)
            {
                _objLoggingProvider.LogException("Error while GetQueueHistory ", ex);
                throw;
            }
        }

        public QueueHistory SaveQueueHistory(QueueHistory queueHistory)
        {
            try
            {
                _objLoggingProvider.LogMessage(LogType.Info, "Start: Save QueueHistory From Queue");
                using var scope = new TransactionScope();

                queueHistory.CreatedDate = DateTime.UtcNow;
                queueHistory.IsActive = true;

                queueHistory = _objPriceListRepository.QueueHistoryRepository.InsertEntity(queueHistory);
                scope.Complete();
                _objLoggingProvider.LogMessage(LogType.Info, "End: Save QueueHistory From Queue");
                return queueHistory;
            }
            catch (Exception ex)
            {
                _objLoggingProvider.LogException("Error while saving QueueHistory From Queue: ", ex);
                throw;
            }
        }
        ...
        public int UserPriceFileSaveConfig(PriceFileSaveConfig UserFileConfig)
        {
            try
            {
                ...
                confObj.UserSESA = UserFileConfig.UserSESA;
                ...
                confObj.CreatedBy = UserFileConfig.UserSESA;
        ...
        public string GetExcelDetails(long id, bool SendEmail, bool showNotFoundMaterials)
        {
            ...
            try
            {
                ...
                _objLoggingProvider.LogMessage(LogType.Info, "USPT_GeneratePriceFileCreation: End with UserConfigSettingID: " + id);
            }
            catch (Exception ex)
            {
            ...
            if (val == "Success")
            {
                var PriceFileHeaderId = _objPriceListRepository.PriceFileHeaderRepository.GetManyQueryable(x=>x.UserConfigSettingID == id).FirstOrDefault().PriceFileHeaderID;
                int PriceFileDetailsCount = 0;//_objPriceListRepository.PriceFileDetailsRepository.GetManyQueryable(x => x.PriceFileHeaderID == PriceFileHeaderId && x.IsFound != showNotFoundMaterials).Count();

                using (ISqlHelper objSqlHelper = new SqlHelper(AppConfig.ConnectionString))
                {
                    string strUpdateQuery = "Select count(*) from [dbo].[TRN_PriceFileDetails] Where PriceFileHeaderID = @PriceFileHeaderID and IsFound = @IsFound";

                    List<SqlParameter> lstParameters = new List<SqlParameter>();                    
                    lstParameters.Add(new SqlParameter() { ParameterName = "@IsFound", Value = !showNotFoundMaterials });
                    lstParameters.Add(new SqlParameter() { ParameterName = "@PriceFileHeaderID", Value = PriceFileHeaderId });

                    PriceFileDetailsCount = int.Parse(objSqlHelper.ExecuteScalarQuery(CommandType.Text, strUpdateQuery, lstParameters.ToArray()).ToString());

                }

                if (PriceFileDetailsCount > 0)
                {
                ...
                {
                    DataResult = "DATA_NOT_FOUND";
                }                
            }
            else
            {
                DataResult = "UNSUCCESSFUL";
                _objLoggingProvider.LogMessage(LogType.Info, "GetExcelDetails: End with UserConfigSettingID: " + id + " and UNSUCCESSFUL - " + val);
            }
                ...
                foreach (var cust in selected_cust)
                {
                    ...
                    try
                    {
                        ...
                        foreach (JObject res in resarray)
                        {
                            ...
                            int autoFitWidth = (int)sheet.GetColumnWidth(headerRowIndexCnt);
            ...
            switch (CountryCode)
            {
                case "AU":
                    switch (PC1)
                    {
                        case "ED":
                        case "EC":
                            template = "AUWholesalerOutputTemplate";
                            break;
                        default:
                            template = "AUOtherChannelOutputTemplate";
                            break;
                    }
                    break;
                case "NZ":
                    switch (PC1)
                    {
                        case "ED":
                        case "EC":
                            template = "NZRebateCustomerOutputTemplate";
                            break;
                        default:
                            template = "NZNonRebateCustomerOutputTemplate";
                            break;
                    }
                    break;
                default:
